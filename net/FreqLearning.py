...
class Freq_De(nn.Module):
    def __init__(self, in_chans, kernel_size=3, stride=1, group=8):
        super(Freq_De, self).__init__()
        self.stride = stride
        self.kernel_size = kernel_size
        self.group = group
        self.lamb_l = nn.Parameter(torch.zeros(in_chans), requires_grad=True)
        self.lamb_h = nn.Parameter(torch.zeros(in_chans), requires_grad=True)
        self.conv = nn.Conv2d(
            in_chans, group * kernel_size**2, kernel_size=1, stride=1, bias=False
        )
...
class FreqLearning(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.split = Freq_De(in_chans=in_ch)
        self.high_pass = ResMoEBlock(
            in_ch=in_ch,
            num_experts=num_experts,
            use_shuffle=use_shuffle,
            lr_space=lr_space,
