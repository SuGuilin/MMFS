...
for i = 1 : length(names)
    ...
    Fused_dir = '../';
    Fused_dir = fullfile(Fused_dir, 'YUVFusion', strcat(cell2mat(names(i)), '_', dataset)); % 融合结果所在文件夹 此处是 'Evaluation\Image\Algorithm\DenseFuse_TNO'
    ...
    FMI_dct_set = []; FMI_w_set = [];
     for j = 1:num
        if (isequal(fileNames{j}, '.') || isequal(fileNames{j}, '..'))
            ...
            if i==1
                fileName_Fusion = strcat(fileName_Fusion);
            end
