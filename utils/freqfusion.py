...
class FreqFusion(nn.Module):
    ...
    def kernel_normalizer(self, mask, kernel, scale_factor=None, hamming=1):
        ...
        # 想不明白维度转换，可参考https://img2023.cnblogs.com/blog/2024073/202305/2024073-20230510163229838-1612791755.png

        mask = mask.view(n, mask_channel, -1, h, w)
        mask = F.softmax(mask, dim=2, dtype=mask.dtype)  # 确保每个像素位置的卷积核内权重和为1
        mask = mask.view(n, mask_channel, kernel, kernel, h, w)
        mask = mask.permute(0, 1, 4, 5, 2, 3).view(n, -1, kernel, kernel)
        # mask = F.pad(mask, pad=[padding] * 4, mode=self.padding_mode) # kernel + 2 * padding
        mask = mask * hamming  # 平滑权重，减少边界效应
        mask /= mask.sum(dim=(-1, -2), keepdims=True)  # 确保每个卷积核权重和为1
        # print(hamming)
        # print(mask.shape)
